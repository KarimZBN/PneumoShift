"""
Avalia a RSNA com 390 pneumonia + 234 normal (mesmo n e proporcao da Chest X-Ray),
repetida por semente.

Le as selecoes dados/splits/rsna_pareado_seed<N>.csv e consulta o cache de scores
(processed/rsna/scores_por_id.csv) — sem reinferencia nem copia de imagens. Limiar 0,5.
Reporta media +- desvio e IC 95% entre as sementes.

Saida em resultados/APOIO_rsna-pareado-cxray/: rodadas.csv, resumo.json, LEIA.md.
...
# --- Configuracao ---
PREFIXO = "rsna_pareado"
SEEDS = list(range(42, 52))
LIMIAR = 0.5

CACHE = paths.RSNA_FULL / "scores_por_id.csv"
SPLITS_DIR = paths.DADOS / "splits"
ROTULO = "pareado a Chest X-Ray (390 pneumonia + 234 normal)"
...
def main():
    ...
    resumo = {"limiar": LIMIAR, "conjunto": PREFIXO, "n_seeds": len(SEEDS),
              ...
              "composicao": "390 pneumonia + 234 normal (pareado a Chest X-Ray)",
              "metricas": {k: media_ic(v) for k, v in coletas.items()}}

    out = paths.pasta_resultado("APOIO", "rsna-pareado-cxray")
    ...
    paths.escrever_leia(
        out, "RSNA — pareado a Chest X-Ray (390/234)",
        "Mesmo tamanho e proporcao por classe da base interna, repetido por semente.",
        "390 pneumonia + 234 normal (mesmo n/proporcao da Chest X-Ray), repetido em "
        f"{len(SEEDS)} sementes; limiar 0,5. Reporta media +- desvio e IC 95% entre seeds. "
        "O pareamento em quantidade nao torna as bases equivalentes.",
